...
void midi_bus_setup(void) {
  llist_builder(&midi_node_stack, &midi_nodes_array[0], MIDI_NODES, sizeof(midi_nodes_array[0])); // Add X nodes to the midi_node_stack
  llist_raz(&midiIn);
  llist_raz(&midiOut);
  llist_raz(&midiChord);
};
...
void printBytes(const uint8_t* data_ptr, size_t data_length) {
