...
// FIXME
bool mapping_grid_interact(blob_t* blob_ptr, common_t* common_ptr) {
  mapp_grid_t* grid_ptr = (mapp_grid_t*)common_ptr;
  for (uint8_t j = 0; j < grid_ptr->params.touchs; j++) {
    if (blob_ptr->centroid.x > grid_ptr->params.rect.from.x &&
      ...
      blob_ptr->action.data_ptr = &grid_ptr->params.keys[j];
      return true;
    }
  }
  return false;
...
void mapping_grid_play(blob_t *blob_ptr) {
    ...
    switch_t* touch_ptr = (switch_t*)blob_ptr->action.data_ptr;
